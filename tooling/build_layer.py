...
scale inside every agent prompt, a project's module-map diagram, and the imperatives
map with its section hashes. Hand-copied derived text drifts (P3); this script renders it
and --check fails CI when any rendering is stale, so the copy cannot drift.
